...
static int globalvar_mmap(struct file *file, struct vm_area_struct *vma);
...
static int hello_init(void)
{
    ...
    */
    ...
    else
    {
        ...
        globalvar.csr=ioremap(0x60060000,1);
	globalvar.buffer_write=ioremap(0x60060100,64);
	globalvar.buffer_read=ioremap(0x60060200,16);
    ...
    */
    ...
    my_class = class_create(THIS_MODULE, "idct_acc");
    device_create(my_class, NULL, dev, NULL, "idct_acc");
...
/*static int __init hello_init(void) 
{
    ...
    led_addr=ioremap(0x60030000,1);
    printk(KERN_EMERG "led_addr=%x\n",(unsigned int)led_addr);
    *led_addr=0x5555;
    return 0; 
} 
*/
...
static ssize_t globalvar_read(struct file *filp,char *buf,size_t len,loff_t *off)
{
    ...
    */
    while((*globalvar.csr&0x02)==0);
    if(copy_to_user(buf,globalvar.buffer_read,len))
    {
...
static int globalvar_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if(remap_pfn_range(vma,
		         vma->vm_start,
		         ((unsigned long)0x60060000)>>PAGE_SHIFT,
		         size,
		         vma->vm_page_prot))
	    {  
...
static ssize_t globalvar_write(struct file *filp,const char *buf,size_t len,loff_t *off)
{
    ...
    if(copy_from_user(globalvar.buffer_write,buf,len))
    {
        //up(&globalvar.sem); //V 操作
        return -EFAULT;
    }
